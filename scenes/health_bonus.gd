...
@export var speed: int = 200  # The speed at which the health bonus falls.
...
func _ready() -> void:
	# Scale the sprite and collision area by 1.2x.
	$Sprite2D.scale *= 1.2  # Increase the size of the sprite by 1.2x.
	$CollisionShape2D.scale *= 1.2  # Increase the collision area by 1.2x.
